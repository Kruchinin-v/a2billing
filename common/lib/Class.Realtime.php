...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~CUSDTOM~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        //Добавить новые поля для webrtc
        $countArray = count($list_names);
        $list_names[$countArray] = "dtlsenable"; $countArray++;
        $list_names[$countArray] = "dtlsverify"; $countArray++;
        $list_names[$countArray] = "dtlscertfile"; $countArray++;
        $list_names[$countArray] = "dtlsprivatekey"; $countArray++;
        $list_names[$countArray] = "dtlssetup"; $countArray++;
        $list_names[$countArray] = "dtlsrekey"; $countArray++;
        $list_names[$countArray] = "avpf"; $countArray++;
        $list_names[$countArray] = "force_avp"; $countArray++;
        $list_names[$countArray] = "icesupport"; $countArray++;
        $list_names[$countArray] = "rtcp_mux"; $countArray++;
        $list_names[$countArray] = "encryption"; $countArray++;
        $list_names[$countArray] = "transport"; $countArray++;

        //получить значение поля fax для клиентов, где должно быть указано webrtc, если оно нужно
        $this -> webrtcStat = new Table("cc_card", 'id, fax' );
        $list =  $this -> webrtcStat -> Get_list($this -> DBHandler, '', null, null, null, null);



        for ( $i = 0; $i < count($list_friend); $i++ ) {
            $webrtcStatus = 0;
            for ($j = 0; $j < count($list); $j++) {
                if ( $list[$j]["id"] == $list_friend[$i]["id"] && $list[$j]["fax"] == "webrtc") {
                    //echo "Клиент " . $list_friend[$i][0] . " использует webrtc";
                    $webrtcStatus = 1;
                }//if
            }//for

            //значит клиенту webrtc не нужен
            if ($webrtcStatus == 0)
                continue;

            $list_friend[$i][count($list_friend[$i])/2] = 'yes';
            $list_friend[$i]['dtlsenable'] = 'yes';
            $list_friend[$i][count($list_friend[$i])/2] = 'fingerprint';
            $list_friend[$i]['dtlsverify'] = 'fingerprint';
            $list_friend[$i][count($list_friend[$i])/2] = '/etc/asterisk/keys/webrtc.prikazchikk.ru.crt';
            $list_friend[$i]['dtlscertfile'] = '/etc/asterisk/keys/webrtc.prikazchikk.ru.crt';
            $list_friend[$i][count($list_friend[$i])/2] = '/etc/asterisk/keys/webrtc.prikazchikk.ru.key';
            $list_friend[$i]['dtlsprivatekey'] = '/etc/asterisk/keys/webrtc.prikazchikk.ru.key';
            $list_friend[$i][count($list_friend[$i])/2] = 'actpass';
            $list_friend[$i]['dtlssetup'] = 'actpass';
            $list_friend[$i][count($list_friend[$i])/2] = '0';
            $list_friend[$i]['dtlsrekey'] = '0';
            $list_friend[$i][count($list_friend[$i])/2] = 'yes';
            $list_friend[$i]['avpf'] = 'yes';
            $list_friend[$i][count($list_friend[$i])/2] = 'yes';
            $list_friend[$i]['force_avp'] = 'yes';
            $list_friend[$i][count($list_friend[$i])/2] = 'yes';
            $list_friend[$i]['icesupport'] = 'yes';
            $list_friend[$i][count($list_friend[$i])/2] = 'yes';
            $list_friend[$i]['rtcp_mux'] = 'yes';
            $list_friend[$i][count($list_friend[$i])/2] = 'yes';
            $list_friend[$i]['encryption'] = 'yes';
            $list_friend[$i][count($list_friend[$i])/2] = 'wss,udp';
            $list_friend[$i]['transport'] = 'wss,udp';
        }
//        var_dump($list_friend[1]);

//        var_dump($list_names);
//        echo count($list_names) . "\n";
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~CUSDTOM~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
